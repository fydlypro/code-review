...
export async function upsertCustomerProfile(payload: {
  ...
  // Ne jamais écraser first_name / phone avec null si non fourni —
  // sinon un simple re-login sans remplir le champ efface le prénom en base.
  // Ne jamais écraser un champ existant avec null si la valeur n'est pas fournie.
  // gdprAccepted=false ou undefined (re-login) ne doit pas supprimer le consentement enregistré.
  ...
  if (payload.gdprAccepted === true) upsertPayload.gdpr_accepted_at = new Date().toISOString();
